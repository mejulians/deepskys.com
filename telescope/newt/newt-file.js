...
function Load_Red( s ) {
	s.spec.title = "Julians Newtonian";
	s.spec.notes = "8.5 inch F5";
	s.spec.unit_meas =  "mm";
	s.spec.primary_diam = 215;
	s.spec.f_ratio = 5;
    s.spec.secondary_minor = 60;
	s.spec.tube_id = 255;
	s.spec.tube_thick = 5;
	s.spec.focus_to_tubefront = 270;
	s.spec.pri_fnt_tube_back = 147;
	s.spec.fixed_baffles = true;
	s.spec.focus_height = 130;
	s.spec.focus_extra = 30;
	s.spec.focus_camera = 0.0;
	s.spec.focus_diam = 50;
	s.spec.eye_fl = [7,12,18,25,0,0,0,0,0,0];
    s.spec.eye_af = [35,60,44,43,0,0,0,0,0,0];
	s.spec.save_date = '';
	s.savename = "Julians Newtonian";
	s.modified = false;
	Scope2Specs( scope );
	resetZoom();
	clearDiagCalc();
